...
if __name__ == "__main__":
    from core.scene_objs.base_scene_obj import BaseSceneObj, SimulationReturn
...
class BaseGlass(BaseSceneObj):
    ...
    def fill_glass(self, canvas_renderer, is_above_light: bool, is_hovered: bool):
        """
    ...
    def refract(
        ...
        if sq1 < 0:
            ...
            brightness_threshold = 1e-6 if self.scene.color_mode != 'default' else 0.01
            ...
            if total_brightness > brightness_threshold:
                ...
                # Adaptive ray sampling for dim rays
                if not ray.gap and self.scene.color_mode == 'default':
            ...
            # Update the incident ray to be the refracted ray
            ray.p1 = {'x': inc_x, 'y': inc_y} if isinstance(ray.p1, dict) else incident_point

            refr_x = inc_x + n1 * ray_x + (n1 * cos1 - cos2) * normal_x
            refr_y = inc_y + n1 * ray_y + (n1 * cos1 - cos2) * normal_y

            if isinstance(ray.p2, dict):
                ray.p2 = {'x': refr_x, 'y': refr_y}
            else:
                ray.p2 = geometry.point(refr_x, refr_y)

            # Use original brightness values, not potentially modified ones
            ray.brightness_s = original_brightness_s * (1 - R_s)
            ray.brightness_p = original_brightness_p * (1 - R_p)

            # Check if refracted ray is bright enough to continue
            refracted_brightness = ray.brightness_s + ray.brightness_p
    ...
    def get_ref_index_at(self, point, ray) -> float:
        """
    ...
    def get_incident_type(self, ray) -> float:
        """
